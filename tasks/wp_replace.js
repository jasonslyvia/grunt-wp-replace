...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('wp-replace', 'replace filereved or concated assets filename', function(){
    ...
    this.files.forEach(function(file){
      ...
      file.src.filter(function(filePath){
        ...
        //进行静态资源引用替换
        var updated = replaceReference(content, cssRegExp, 'css', cssPath, newFiles, concat);
        updated = replaceReference(content, jsRegExp, 'js', jsPath, newFiles, concat);
      ...
      var concated = concat.every(function(el, i){
        ...
        if(el.src.join(' ').indexOf(asset) !== -1){
          ...
          //否则将concat后对应的文件替换，同时添加已替换属性
            ...
            //若指定的concat后的文件不存在
            if (!grunt.file.exists(filePath+revvedFiles[el.dest])) {
